...
export const hashPassword = SecurityService.hashPassword;
export const comparePasswords = SecurityService.comparePasswords;
  ...
  passport.use(
    new LocalStrategy(async (username, password, done) => {
      try {
        ...
        const isValidPassword = await SecurityService.comparePasswords(password, user.password);
        if (!isValidPassword) {
          return done(null, false, { message: "Invalid username or password" });
        }

        // Check if password needs rehashing with stronger parameters
        if (SecurityService.needsRehash(user.password)) {
          try {
            const newHash = await SecurityService.hashPassword(password);
            await storage.updateUser(user.id, { password: newHash });
          } catch (error) {
            console.error("Failed to rehash password:", error);
